...
function buildSelection(
  ...
  const selection: ApplyPresetSelection = []
  tree.forEach((category, c) => {
    const families: ApplyPresetSelection[number]['families'] = []
    category.families.forEach((family, f) => {
      const variantIndices: number[] = []
      family.variants.forEach((_, v) => {
        if (checked.has(variantKey(c, f, v))) variantIndices.push(v)
      })
      if (variantIndices.length > 0) families.push({ familyIndex: f, variantIndices })
    })
...
export function CatalogPresetPicker({
  ...
  const loadPresets = useCallback(async () => {
    ...
    setLoadError(null)
    try {
      const data = await fetcher('/api/catalog-presets')
  ...
  const onApply = async () => {
    if (!selected || selection.length === 0) return
    setApplying(true)
    setApplyError(null)
    try {
      const result = await apply(selected.id, selection)
      onApplied(result)
    } catch (err) {
      setApplyError(
        err instanceof Error ? err.message : 'افزودن قالب انجام نشد',
      )
    } finally {
      setApplying(false)
    }
