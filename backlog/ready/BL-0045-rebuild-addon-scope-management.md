---
id: BL-0045
title: Rebuild add-on scope management
status: ready
type: feature
priority: high
size: medium
created: 2026-06-30
updated: 2026-06-30
---

## Parent

[BL-0041 Service catalog big-bang migration](BL-0041-service-catalog-big-bang-migration.md)

## What to build

Move ServiceAddon setup and lookup to the simplified scope model so managers can make add-ons available everywhere, by ServiceCategory, or by ServiceVariant, and appointment add-on selection still behaves correctly.

## Acceptance Criteria

- [ ] Add-on create/update accepts `all`, `category`, and `service` scopes only.
- [ ] Family-scoped legacy add-ons are represented as equivalent service scopes after migration.
- [ ] Add-ons with no legacy scope are available everywhere after migration.
- [ ] Manager and setup APIs can save and read the simplified scopes.
- [ ] Appointment add-on lookup returns active add-ons available for the selected ServiceVariant.
- [ ] API and UI tests cover global, category, service, and migrated family-scope behavior.

## Blocked by

- [BL-0042 Migrate legacy combos and add-on scopes into package-ready storage](BL-0042-migrate-legacy-combos-and-addon-scopes.md)
- [BL-0043 Replace service catalog API contracts](BL-0043-replace-service-catalog-api-contracts.md)

## Notes

- Type: AFK.
- Source slice from `SERVICE_CATALOG_MIGRATION_PLAN.md`.
