...
- [ ] Public salon pages list active, public-visible ServiceVariants only.
- [ ] Public package definitions are not exposed through public page settings or public booking APIs.
- [ ] Public AppointmentRequest creation still snapshots the selected ServiceVariant name, duration, and price.
- [ ] AppointmentRequest approval remains compatible with the service-only public request path.
- [ ] Existing appointment create/update with add-ons still works after the add-on scope rewrite.
- [ ] Regression tests cover public service listing, request creation, approval, and snapshot stability.
